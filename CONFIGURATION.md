...
`0x30` or `0xD0`; `0x30` is the default and matches the bundled
`BertSCSI-v2-30h-37h.ROM`. Use `0xD0` with the bundled
`BertSCSI-v1-D0h-D7h.ROM`. `[media]` keys `scsi_image` and `scsi_image_mode`
attach its raw disk as read-only or read-write. Target ID 0 is the default.
The ROM path and image chooser keep independent directory histories. See
[`SCSI.md`](SCSI.md) for BERT-specific partition requirements.
