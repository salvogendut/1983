...
/* ---- Optional sibling legacy cycle tables ---------------------------
 ...
 *
 * These tables are retained to keep the core comparable with its 1984
 * sibling, but they include CPC-oriented bus timing and are disabled by
 * default in 1983. The normal path uses the instruction implementation's
 * standard Z80 cycle counts.
 * ------------------------------------------------------------------ */
/* I/O totals used only by the optional sibling legacy timing path. */
...
/* Public z80_step: use standard instruction counts by default, with an
 * opt-in path for comparing against the sibling legacy cycle tables. */
int z80_step(Z80 *cpu, Z80Bus *bus) {
    /* The inherited tables model CPC bus timing. They are useful for
     * cross-sibling diagnostics only; MSX execution uses standard Z80
     * instruction timing unless explicitly opted in. */
    static int use_tables = -1;
    if (use_tables == -1) {
        const char *e = getenv("MSX_Z80_LEGACY_TABLES");
        use_tables = (e && e[0] == '1') ? 1 : 0;
    }
...
static int z80_step_impl(Z80 *cpu, Z80Bus *bus) {
    ...
    if (cpu->pending_irq && cpu->iff1 && !cpu->ei_delay) {
        ...
        /* Standard Z80 interrupt acknowledge and vectoring costs. */
        cpu->iWSAdjust = 0;
        switch (cpu->im) {
            case 0: case 1:
                cpu->pc = 0x0038;
