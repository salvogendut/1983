...
typedef struct {
    ...
    bool halted;
    bool pending_irq;
    /* Set true by z80_step when it accepts a maskable interrupt. The
     * machine scheduler consumes this by clearing it before the next step. */
    bool int_accepted;

    int cycles;        /* T-states consumed this step */
    u8   last_op;      /* opcode just fetched by z80_step */
    ...
    /* Pipeline hint retained for compatibility with the sibling core's
     * optional legacy timing tables. It is not used by normal MSX timing. */
    int iWSAdjust;
} Z80;

/* Bus callbacks supplied by the emulated machine. */
typedef struct {
