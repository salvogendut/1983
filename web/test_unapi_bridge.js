...
const callbacks = [];
const wasmModule = {
  _poc_unapi_dns_result: (...args) => callbacks.push(["dns", ...args]),
  _poc_unapi_tcp_open_result: (...args) => callbacks.push(["tcp", ...args]),
  _poc_unapi_udp_open_result: (...args) => callbacks.push(["udp", ...args]),
};

const bridge = new UnapiBridge({
  WebSocketCtor: FakeWebSocket,
  endpoint: "ws://127.0.0.1:1983/unapi",
});
...
assert.equal(bridge.tcpAvailable(0), 0);
...
assert.equal(bridge.udpAvailable(1), 0);

assert.equal(bridge.takeActivity(), true);
assert.equal(bridge.takeActivity(), false);
bridge.setDevice(false);
assert.equal(bridge.isConnected(), false);

async function testRequestTimeout() {
  const timedCallbacks = [];
  const timedBridge = new UnapiBridge({
    WebSocketCtor: FakeWebSocket,
    endpoint: "ws://127.0.0.1:1983/unapi",
    requestTimeoutMs: 10,
  });
  timedBridge.attachModule({
    _poc_unapi_tcp_open_result: (...args) => timedCallbacks.push(args),
  });
  timedBridge.setDevice(true);
  ...
  timedBridge.setDevice(false);
}

testRequestTimeout().then(() => {
  console.log("UNAPI browser bridge tests passed");
}).catch(error => {
  console.error(error);
  process.exitCode = 1;
});
