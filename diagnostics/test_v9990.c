...
static void write_register(MsxV9990 *v9990, u8 reg, u8 value) {
    assert(v9990_io_write(v9990, 0x64, (u8)(0x80 | reg)));
    assert(v9990_io_write(v9990, 0x63, value));
}

static u8 read_register(MsxV9990 *v9990, u8 reg) {
    u8 value = 0;
...
static void write_vram(MsxV9990 *v9990, unsigned address,
...
static void test_enable_and_ports(void) {
    ...
    assert(v9990_set_enabled(&v9990, true, 1) == 0);
    ...
    assert(v9990_set_enabled(&v9990, false, 0) == 0);
    assert(!v9990.enabled);
    assert(!v9990_io_read(&v9990, 0x60, &value));
    v9990_destroy(&v9990);
}
